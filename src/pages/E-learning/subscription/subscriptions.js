...
import { deleteSubscription } from '../../../redux/actions/E-LearningActions/subscriptions';
...
export default function WishList() {
    ...
    const fetchWishList = async () => {
        try {
            const { data } = await api.fetchSubscriptionByIdUser(userId);
            setWishList(data)
        } catch (error) {
            console.log(error);
        }
    ...
    const handleDeleteWishes = (selected) => {
        ...
        selected.map((wishId) => dispatch(deleteSubscription(wishId)));
        setSelected([]);
